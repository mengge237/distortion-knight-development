...
| # | .asset文件名 | 菜单路径 | 参数设置 | 用途 |
|---|---|---|---|---|
| 1 | SlimeInherentEffect | MutationChess/Inherent/Slime | — | 粘液字段固有 |
| 2 | ReluctantInherentEffect | MutationChess/Inherent/Reluctant | drawCount=1 | 不舍字段固有 |

> **注意**：固有效果文件名必须以 `Effect` 结尾（与 effectId 一致），否则 CardData.LoadEffect 会报 "未找到效果" 错误。
...
### 2.2 需要新建的卡牌（62张 · **稀有度不含Mythic，Mythic→Legendary）
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| 28 | 腐化蔓延 | Attack | Common | 1 | 7 | 0 | 0 | Corrupt | false | DealDamageEffect | 造成7点伤害。腐化系基础攻击 |
| 29 | 腐化之触 | Defense | Common | 1 | 0 | 6 | 0 | Corrupt | false | ApplyBlockEffect | 获得6点格挡。腐化系基础防御 |
| 30 | 腐化 | Attack | Common | 0 | 6 | 0 | 0 | Corrupt | true | DealDamageEffect | 造成6点伤害。消耗 |
| 31 | 腐蚀打击 | Attack | Uncommon | 1 | 14 | 0 | 2 | Corrupt | true | DealDamageEffect, ApplyVulnerabilityEffect | 造成14点伤害，施加2层易伤。消耗 |
| 32 | 腐化释放 | Skill | Rare | 2 | 0 | 0 | 0 | Corrupt | false | CorruptReleaseEffect | 消耗所有腐化卡，每张触发效果 |
| 33 | 暗影腐化 | Skill | Rare | 1 | 0 | 0 | 2 | Corrupt | false | AddCardToDeckEffect | 向牌组添加2张腐化卡 |
| 34 | 腐化吞噬 | Attack | Legendary | 3 | 20 | 0 | 2 | Corrupt | true | DealDamageEffect, DrawCardsEffect | 造成20点伤害，抽2张牌。消耗。Legendary |
| **注** | | | 原Mythic→Legendary | | | | | | | |
...
## 六、美术资源校验清单（2026-07-30 校验）

> 校验范围：`Assets/TextMesh Pro/Resources/` 下所有美术 PNG 资源与 .asset 配置的匹配情况

### 6.1 卡牌插画 CardArt（68/72 ? · 4缺失 · 1已修复）

保存目录：`Assets/TextMesh Pro/Resources/CardArt/`

| 状态 | 数量 | 说明 |
|---|---|---|
| 已有插画 | 68 | 与卡牌 .asset 同名匹配 |
| 缺失插画 | 3 | 见下表 |
| 已修复命名 | 1 | 隐刃.png → 影刃.png（原名与卡牌名不一致） |

**缺失插画清单（3张）**

| 卡牌名 | 系列 | cardArtPath | 状态 |
|---|---|---|---|
| 暗影爆发 | 暗影 | CardArt/暗影爆发 | PNG 缺失，需补充 |
| 腐化之触 | 腐化 | CardArt/腐化之触 | PNG 缺失，需补充 |
| 腐化蔓延 | 腐化 | CardArt/腐化蔓延 | PNG 缺失，需补充 |

> **说明**：卡牌 art 缺失时 CardUI 会静默禁用插画显示（已移除 LogWarning），不影响游戏运行。

### 6.2 遗物插画 RelicsArt（49/50 ? · 1缺失）

保存目录：`Assets/TextMesh Pro/Resources/RelicsArt/`

| 状态 | 数量 | 说明 |
|---|---|---|
| 已有插画 | 49 | 与遗物 .asset 同名匹配 |
| 缺失插画 | 1 | 不舍锁链（Boss遗物 B5） |

**缺失插画清单（1个）**

| 遗物名 | relicId | 派系 | 状态 |
|---|---|---|---|
| 不舍锁链 | Boss_ReluctantChain | 不舍 | PNG 缺失，需补充 |

### 6.3 敌人立绘 EnemySprites（11/11 ? 全部完成）

保存目录：`Assets/TextMesh Pro/Resources/EnemySprites/`

| 类型 | 数量 | 文件 |
|---|---|---|
| Boss | 3 | 深渊之主、深渊之主·克苏鲁之影、腐化君王 |
| Elite | 4 | 地狱审判官、腐化巨兽、腐蚀骑士、虚空巫师 |
| Normal | 4 | 深渊蛆虫、畸变猎犬、瘟疫侍僧、腐化士兵 |

### 6.4 战斗背景 BattleBackgrounds（3/3 ? 全部完成）

保存目录：`Assets/TextMesh Pro/Resources/BattleBackgrounds/`
...
### 6.5 角色立绘 PlayerSprites（2/2 ? 全部完成）

保存目录：`Assets/TextMesh Pro/Resources/PlayerSprites/`
...
### 6.6 界面UI InterfaceUI（22/22 ? 全部完成）

保存目录：`Assets/TextMesh Pro/Resources/InterfaceUI/`

涵盖：卡牌背景、能量背景、商店背景、战斗日志、顶部栏、奖励面板等 22 张 UI 元素。

### 6.7 效果资源 Effects（119个 ? 全部完成）

保存目录：`Assets/TextMesh Pro/Resources/Effects/`（119个 .asset）
保存目录：`Assets/TextMesh Pro/Resources/InherentEffects/`（2个 .asset）

- 全部 121 个效果 .asset 均为 UTF-8 with BOM 编码，无乱码
- 全部 .asset 的 m_Script GUID 与对应 .cs.meta 匹配
- 包含本指南第一章列出的全部效果，以及额外历史遗留效果（均有效）

### 6.8 药水资源 Potions（未创建）

> **待办**：项目中已有药水系统代码（PotionDropService、PotionDataAsset、最大容量3），但 `Resources/Potions/` 和 `PotionArt/` 目录尚未创建。需要补充：
> - `Resources/Potions/` — 药水 .asset 配置文件
> - `PotionArt/` — 药水插画 PNG
...
| 资源类型 | 已有 | 应有 | 完成率 |
|---|---|---|---|
| 卡牌插画 CardArt | 68 | 72 | 94.4% |
| 遗物插画 RelicsArt | 49 | 50 | 98.0% |
| 敌人立绘 EnemySprites | 11 | 11 | 100% |
| 战斗背景 BattleBackgrounds | 3 | 3 | 100% |
| 角色立绘 PlayerSprites | 2 | 2 | 100% |
| 界面UI InterfaceUI | 22 | 22 | 100% |
| 效果配置 Effects | 121 | 121 | 100% |
| 药水资源 Potions | 0 | 待定 | 0% |
| **总计** | **276** | **281+** | **98.2%** |
